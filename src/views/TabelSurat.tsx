...
import { Search, FileText, Download, Archive } from 'lucide-react'
...
interface TabelSuratProps {
  ...
  onArchive: (row: LetterRequestRow) => void
  busyId: string | null
}

const GRID = '0.85fr 1fr 0.35fr 0.8fr 0.6fr 0.75fr 0.75fr 0.65fr 0.75fr 0.7fr 0.45fr'

export function TabelSurat({ requests, loading, onApprove, onMarkTTD, onRevisi, onOpenDetail, onArchive, busyId }: TabelSuratProps) {
    ...
    <div className="page-container">
            ...
            <div className="flex items-center justify-between pt-3" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-2">
                ...
                {(r.status === 'pending' || r.status === 'revisi') && (
                  <button onClick={() => onArchive(r.raw)} disabled={busyId === r.id} title="Arsipkan surat"
                    className="w-9 h-9 rounded-xl border flex items-center justify-center disabled:opacity-50"
                    style={{ borderColor: 'var(--card-border)', color: 'var(--text-muted)', background: 'rgba(107,114,128,0.08)' }}>
                    <Archive size={15} />
                  </button>
                )}
              </div>
      ...
      <div className="card overflow-x-auto p-0 hidden md:block">
        <div className="min-w-[1100px]">
          ...
          >
            {['Requester', 'Jenis Surat', 'KOP', 'Diajukan', 'Deadline', 'No. HP PIC', 'Nomor Surat', 'RAW Files', 'Download Files', 'Status', 'Arsip'].map((h) => (
